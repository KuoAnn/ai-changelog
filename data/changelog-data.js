/* =============================================================================
  ...
  const DATA_CC = [
    {v:"2.1.250", date:"2026-08-28", cat:"Performance/Bug Fix", body:"修復：官方僅標示為錯誤修復與可靠性改善，未提供逐項細節。"},
    {v:"2.1.248", date:"2026-08-27", cat:"Permissions/Security", body:"新增：<b><code>--restricted</code> 旗標</b>（或 <code>CLAUDE_CODE_RESTRICTED=1</code>）：移除可執行指令或程式碼的內建工具與 <code>WebFetch</code>（除非列在 <code>--tools</code> 中），檔案工具限制在工作目錄內，拒絕 <code>bypassPermissions</code>，並忽略使用者、專案與本機設定檔；agent frontmatter 新增 <code>experimental.cacheTtl</code>（<code>\"5m\"</code> 或 <code>\"1h\"</code>）：未設定 subagent TTL 時套用的逐 agent prompt cache TTL；<b><code>claude self-hosted-runner --client-label &lt;label&gt;</code></b>（或 <code>SELF_HOSTED_RUNNER_CLIENT_LABEL</code>）可覆寫 runner 註冊使用的標籤（預設為主機名稱）；<b>受管設定診斷強化</b>：設定載入失敗時啟動即顯示警告，<code>/doctor</code> 與 <code>/status</code> 亦會說明載入失敗或未抓取的原因（Bedrock/Vertex/第三方供應商、自訂 <code>ANTHROPIC_BASE_URL</code>）；<code>/web-setup</code> 於 GitHub CLI token 缺少 <code>workflow</code> scope 時新增警告，避免推送超大型 repository 被拒；新增 <b><code>/usage-credits</code></b>：AWS Marketplace、自助或試用 Enterprise 組織成員可向管理員申請提高用量上限；<b>跨 session 傳訊（<code>SendMessage</code>／<code>ListAgents</code>）</b>現支援 Bedrock、Vertex、Foundry 環境與關閉遙測時同機 session 互傳訊息。修復：長 session 中每小時約一次的 prompt cache miss（並遺失 extended-thinking context），起因 OAuth token 更新後工具定義被重新渲染；帳號進入用量超額時 <code>ScheduleWakeup</code> 工具定義在 session 與 <code>--resume</code> 之間不一致，導致接續 session 首輪整份 prompt cache miss；Claude Desktop 與 Cowork session 30 天後消失，transcript 清理現保留桌面版仍在使用中的 session（除非組織政策管理保留期），新設定 <code>desktopSessionCleanupPeriodDays</code> 可限制豁免範圍；另一 Claude Code 程序持有 token 更新鎖、該 session token 已過期時被導向登入畫面，現改為可重試錯誤；Windows 上離開 session 或於 win32-input-mode 終端機分頁啟動後 <code>claude agents</code> 清單對鍵盤無反應；無法使用建議登入方式的機器（如已設 <code>ANTHROPIC_API_KEY</code> 或 API key helper）上 <code>/login</code> 建議的 Console 登入會先以 OAuth 錯誤失敗才顯示登入網址，現改為直接退回 API key 登入；<code>/model</code> 與快速模式切換通知的模型名稱未以程式碼樣式呈現，導致 <code>[1m]</code> 等後綴顯示成連結；設定 <code>CI</code> 環境變數時 <code>claude agents</code> 略過 workspace 信任提示；PR 狀態快取含格式錯誤項目時 <code>claude agents</code> 啟動即當機；agent view 於機器關機數週後復活舊背景 session，現改為顯示其真實結束時間，開啟時會先詢問才恢復對話；啟動新 session 時 agent view 有時開啟較舊對話並遺失已輸入的提示；<code>claude agents</code> 開啟已在其他終端機恢復的已停止 session 不再啟動第二個程序，該列會顯示已在終端機中開啟；worktree 分支已合併進目前簽出的預設分支但尚未推送時，<code>claude agents</code> 與 <code>claude rm</code> 誤拒刪除該 session；<code>PermissionRequest</code> 或 <code>PreToolUse</code> hook 印出無效答案時背景 session 靜默卡住，現 <code>claude agents</code> 該列會標明是哪個 hook 與 schema 錯誤；<code>/mcp</code> 誤將宣告 claude.ai connector 類型的專案 <code>.mcp.json</code> 項目列在可信的「claude.ai」標題下，現改列在其真實範圍；<code>headersHelper</code> 提供 <code>Authorization</code> header 的 MCP 伺服器遇 401 時誤走 OAuth discovery 而非依文件重跑 helper 重試；受管設定安全核准對話框需要時 <code>/login</code> 到 Claude apps gateway 會卡住；僅設定 <code>apiKeyHelper</code> 為唯一憑證時 gateway model discovery（<code>CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY</code>）永不執行；<code>claude logs</code> 結束後仍讓執行的終端機保持滑鼠追蹤、bracketed paste 與替代畫面開啟；<code>/ultrareview</code> 與本機種子雲端 session 會將未提交的 <code>prod.env</code> 類與 <code>*.tfvars</code> 檔案，或編輯器暫存、swap、備份的憑證檔複本（如 <code>key.pem.tmp</code>、<code>id_rsa.swo</code>）上傳，現改為保留在本機。改善：Workflow 工具的提示占用大幅縮減（描述由約 5.7k token 降至約 1k token），撰寫腳本的參考資料移入內建 <code>workflow-authoring</code> skill；提示列 PR 徽章在 PR 未變動時降低檢查 GitHub 的頻率，推送或執行 <code>gh pr</code> 指令仍會立即刷新；受管設定中的 client-side timeout、MCP 啟動模式與 stream-watchdog 環境變數不再觸發設定核准提示；<code>/ultrareview &lt;PR#&gt;</code> 於啟動前先確認你的 GitHub 帳號可存取該 repository 並說明修正方式，而非等雲端 session 啟動後才失敗。變更：agent view 派送輸入框的 shift+enter 改為插入換行（與提示框一致），ctrl+enter 改為派送並附加；<code>/loop</code> 自行調節節奏的動態模式與免提示自主模式預設一律可用，含 Bedrock/Vertex/Foundry。"},
    {v:"2.1.247", date:"2026-08-26", cat:"UI/UX", body:"新增：<b>SendFeedback 工具</b>：session 出錯時 Claude 可草擬意見回饋報告，供你於 <code>/feedback</code> 檢視後送出（可用 <code>feedbackDrafts</code> 設定關閉）；<b>spinnerTipsOverride 擴充</b>：新增 <code>{id, text, cooldownSessions, priority}</code> 項目、<code>tipsFile</code> 與 <code>label</code>，組織可將自訂提示與內建提示輪替顯示；Bash 權限提示新增指向 auto mode 的提示，附一鍵「Yes, and switch to auto mode」選項；<b><code>/claude-api cost-optimize</code></b>：分析既有專案的 Claude API 花費並逐項檢視省成本手段（caching、token 精簡、batch、effort、模型選擇）；更新 <code>/claude-api</code> skill 涵蓋 Admin API（組織成員、邀請、workspace、API key、rate limit 報表、workload identity federation、CMEK）。修復：歷史搜尋、<code>/config</code>、<code>/mcp</code>、<code>/skills</code>、背景任務與 <code>/model</code> 中快速按方向鍵接 Enter 會誤動作到游標移動前的那一列；sub-agent 首次呼叫模型即遇 404 時會直接死亡，現改為使用 session 的後備模型鏈，回傳給主 session 的錯誤含錯誤類型、狀態碼、request id 與模型名稱；hook 或背景 agent 印出數 MB 錯誤輸出可能塞爆對話並讓 session 卡在「Prompt is too long」；kitty-protocol 終端機下非拉丁（如西里爾）鍵盤配置的 Ctrl 快捷鍵失效；滑鼠回報剛好在跳脫前綴後分段送達時，類似 <code>&lt;35;150;7M</code> 的文字被插入提示框；Bash 沙箱指令後清理作業會刪除以 dotfile 工具（nix/home-manager、stow）管理、被重新指向沙箱可寫區域外的 <code>~/.claude/settings.json</code> 符號連結；<code>/terminal-setup</code> 會整份覆寫 Zed 的 <code>keymap.json</code> 而非合併寫入按鍵綁定；session 名稱登錄無法更新時 <code>/rename</code> 靜默視為成功，現會提示其他 session 可能仍顯示舊名稱；以 <code>--agent</code> 啟動的 session 執行 <code>/compact</code> 或「Summarize from here」時改用預設系統提示而非該對話自身的提示；背景 session 的終端機主機程序當機後 <code>claude agents</code> 永遠顯示「opening…」，現改為數秒內顯示失敗原因並可按 Enter 重啟；hook 或背景任務輸出檔案無法寫入時記憶體用量無限成長，現會在檔案中註明輸出遺失位置；透過 SSH 執行 <code>/install-github-app</code> 時複製捷徑現會說明登入網址實際如何被複製而非一律宣稱成功，且無瀏覽器可開啟時網址會立即顯示；前景延續到背景 session 的 shell 指令結束時記錄內部錯誤或顯示誤導性的 <code>[exited with code -1]</code> 訊息；無版本號 marketplace plugin 的即時快取目錄於第二次不同範圍安裝時被刪除重建，可能干擾正在使用該 plugin 的 session；以 <code>/remote-control</code> 啟動的 Remote Control session 未回報工作目錄 diff 給已連線的客戶端；自架 runner session 在 Claude Code 尚未啟動前回報 <code>running</code>，可能觸發 Claude desktop app 過早顯示「Claude is waiting for your input」通知；受管設定同時要求 Claude apps gateway 登入且 Anthropic 端點不可連線時，首次執行設定以「Unable to connect to Anthropic services」結束；傳送訊息後立即切換權限模式的雲端 session（Claude Code on the web、桌面與行動版）偶爾顯示前一個權限模式；雲端 session 容器在回合之間重啟、而背景 agent、shell 或監控仍在執行時 session 會失去回應，現恢復的 session 會回報遺失的工作內容。改善：plugin marketplace 強化：名稱含控制或不可見字元的一律拒絕，<code>/plugin</code> 與 <code>claude plugin</code> 輸出中 marketplace 提供的文字改為跳脫安全；Bedrock、Vertex、Foundry（及任何關閉遙測的部署）現會告知 Claude 設定的 MCP 伺服器連線失敗，而非讓 Claude 誤判其工具不存在。變更：Sonnet 5 預設 auto-compact 視窗改為完整 1M context，1M 視窗的 session 現於約 967K token 才自動壓縮（原約 934K）；跨 session 傳訊預設摺疊成單行 <code>Message from @&lt;sender&gt;: &lt;first line&gt;</code> 預覽，Ctrl+O 可展開完整內容；渲染後的 markdown 終端機超連結：連結指向網路或 automounter 路徑、含控制字元或以不可見字元開頭時，改為顯示為純文字；提示列的 PR 徽章在終端機重新取得焦點時，若上次檢查未滿一分鐘則略過 GitHub 重新檢查；受管設定強制 gateway 登入或自訂 OAuth 部署時，分析功能從啟動起即保持關閉，不僅限登入後；Claude apps gateway 登入請求新增識別 Claude Code 的參數（<code>surface=claude_code</code> device-authorization 參數與 <code>claude-code/&lt;version&gt;</code> User-Agent）；管理員的受管設定無法讀取時，組織登入強制機制改為於啟動時即結束，即使主機提供或使用者層級的 Windows 登錄設定存在亦然。"},
    {v:"2.1.246", date:"2026-08-25", cat:"Permissions/Security", body:"新增：<b>Bash 萬用字元警告</b>：子指令前含萬用字元的 Bash 允許規則（如 <code>Bash(git * main)</code>）啟動時新增警告，因該規則同樣會比對到插在子指令前的選項；<b><code>/permissions</code> 新增 Auto mode 分頁</b>：可檢視與編輯 auto mode 分類規則；<b>回合完成時間顯示</b>：回合耗時列尾端新增完成時刻，例如 <code>✻ Sautéed for 23s · done 6:05 PM</code>。修復：全螢幕模式在調整終端機視窗大小後顯示空白 transcript，並卡在捲到底部直到下一次按鍵才恢復；diff 內含極長單行（如 base64 字串）時 transcript 嚴重卡頓，現改為截斷顯示並標記；位於較早訊息時全螢幕捲動異常，包含跳至底部卡在 transcript 中途；啟動目錄已被刪除、機器曾睡眠或主機啟動程序較慢時，背景 session 於 45 秒後開啟失敗；另一個 Claude Code 程序正在重裝 npm 套件期間，背景 session 以「Couldn't start the background service … EACCES」開啟失敗；訊息前 500 字元不含 markdown 語法時，整則訊息的 markdown 渲染被停用，<code>+</code>／<code>N)</code> 清單與 setext 標題亦受影響；headless／remote session 中，被新進訊息中斷的 MCP 工具呼叫回報給模型為「completed with no output」而非明確的中斷錯誤；參數 schema 為空（<code>{}</code>）時，MCP 工具引數被以 JSON 字串傳送而非其實際型別；執行中途被中斷的指令顯示為「Ran 1 shell command」，未標示已被截斷；dynamic workflow 執行中按 ← 或執行 <code>/background</code> 會重啟已完成的 subagent，現改為先詢問並告知將重啟的 subagent 數量；於 <code>claude agents</code> 開啟剛啟動、其 worker 仍在初始化的 session（Windows 常見）會以「was stopped while the respawn was in flight」中止；<code>claude agents</code> 將背景中的具名 session 重複列出，再次將同一對話放入背景時，新一列現會編號（如 <code>my-session (2)</code>）；背景保留清理作業會刪除使用者自建、恰好被舊背景 session 紀錄指到的 <code>.claude/worktrees/</code> 下 git worktree；auto mode 工具呼叫在極大型 session 上被拒為「temporarily unavailable」，現改為依提示大小調整安全檢查期限；plugin 快取為同一 plugin 建立重複的 SHA 命名目錄；frontmatter <code>name</code> 已含 <code>&lt;plugin&gt;:</code> 前綴的 plugin skill，在斜線選單中該前綴重複顯示（如 <code>/plugin:plugin:skill</code>）；以裸名稱（非完整名稱）執行 <code>claude plugin update</code> 對已安裝 plugin 失敗；<code>plugin.json</code> 以含 UTF-8 BOM 儲存時 plugin 安裝失敗；plugin 的 skill 定義於 <code>skills/*/SKILL.md</code> 下時，<code>/reload-plugins</code> 回報 0 個 skill；hook 錯誤訊息顯示未解析的字面 <code>${CLAUDE_PLUGIN_ROOT}</code>，而非實際的 plugin 路徑；<code>/rename</code> 會把主題的提示框邊框顏色（含自訂主題的 <code>promptBorder</code>）覆蓋成預設青色，現改為保留主題色，除非另用 <code>/color</code> 指定；自訂主題的 diff 顏色（<code>diffAdded</code>／<code>diffRemoved</code> 及其淡化版本）在 diff 與 <code>/theme</code> 預覽中被忽略；<code>keybindings.json</code> 中動作名稱錯誤的綁定會靜默讓該按鍵失效，現改為略過該綁定以保留預設功能，並於 <code>--debug</code> 下記錄警告；位於 UTC 東側時區時，<code>/stats</code> 活動熱力圖每日格位偏移一格（週日的計數顯示在週一底下）；從已分叉或背景中的 session 執行 <code>/fork</code> 會以空白對話啟動新 session；以 <code>/--</code> 開頭的提示（如 Lean 文件註解）被誤判為未知斜線指令而拒絕，未送交 Claude；輸入文字不再符合任何實際路徑後，<code>@</code> 檔案選取器仍保持開啟；切到 agents 檢視再切回後，狀態列的花費與耗時被重置為零；全螢幕模式下僅為讓終端機視窗重新取得焦點而點擊，卻把鍵盤焦點移到滑鼠所在的控制項；補全字串或工作目錄含 null byte 時路徑補全失敗；Windows／macOS：非正常結束的 session 留在 <code>~/.claude/sessions</code> 的過期項目未被 headless session 清除；第三方 Anthropic 相容端點（<code>ANTHROPIC_BASE_URL</code>）串流出無 <code>id</code> 的 <code>tool_use</code> 區塊時，UI 於首次工具呼叫即以渲染錯誤中止；覆寫極大既有檔案後，Write 工具回報「Out of memory」或長時間凍結，即使檔案其實已寫入成功；<code>~/.claude/plugins/known_marketplaces.json</code> 為空或損毀時，<code>claude plugin install &lt;name&gt;</code> 靜默結束（或在終端機中卡住）而未回報錯誤；已儲存歷史含 Anthropic API 不接受的工具區塊（常見於第三方 API proxy 寫入）時，恢復的 session 每一輪皆以 400 失敗；部分採用伺服器管理設定的 Team／Enterprise 使用者執行 <code>curl -fsSL https://claude.ai/install.sh | bash</code> 以「Raw mode is not supported」失敗；以 plan mode 結束的 session 在 VS Code 擴充功能中恢復、以及在未設定權限模式時透過 <code>claude -p --continue</code>／<code>--resume</code> 搭配權限提示工具恢復，皆會跳出 plan mode；沙箱「Network request outside of sandbox」權限提示等待期間，<code>Notification</code> hook 未觸發；Bash 權限檢查現對含未閉合 <code>&amp;&amp;</code> 或 <code>||</code> 運算子的格式錯誤指令一律要求核准；<code>--strict-mcp-config</code> session 會要求核准實際上永不載入的 <code>.mcp.json</code> 伺服器，導致背景 session 卡在啟動階段等待；遙測與計量請求誤帶第三方 gateway（<code>ANTHROPIC_BASE_URL</code>）設定的 API key 送往 Anthropic，現改為憑證只送往其對應主機；<code>apiKeyHelper</code> 回傳短效 JWT 時，閒置後第一次提示會顯示明顯的 API 錯誤，現改為送出前先更新過期的快取 token，401/403 認證錯誤則安靜重試；全螢幕與 Ctrl+O transcript 檢視隨 session 長度增長記憶體用量增加，現改為每列渲染訊息不再各自保留一份完整的 transcript 工具查詢表；同一 repository 同時啟動的 <code>/ultrareview</code> 與雲端 session（如來自多個 worktree）有時會以另一次啟動的未提交變更起始；背景雲端 session（如 <code>/autofix-pr</code>）顯示的任務進度計數（如 <code>3/5</code>）偶爾少算一項；Remote Control session 在 claude.ai 與 Claude app 中於第二則提示前仍顯示佔位名稱，現改為第一則提示後就顯示自動產生的標題；標記 <code>requiresUserInteraction</code> 的 MCP 工具，其權限提示仍提供「Yes, and don't ask again」，該選項寫入的允許規則實際上該工具會忽略；自架 runner 於工作輪詢回應格式錯誤（如攔截 proxy 回傳的 HTML 頁面）時結束其進行中 session 或直接退出，現改為重試輪詢；指令沙箱的檔案系統設定未遵循 <code>--setting-sources</code>。改善：<b><code>/cd</code> 改善</b>：切換目錄後，新目錄的專案設定、hook、<code>.mcp.json</code> 伺服器（仍經一般核准提示）、skill 與 agent 現於切換後立即生效，不必等到 <code>--resume</code>；<b>Bash 工具延遲改善</b>：改為重播 snapshot 函式，不再為每個函式起一次 base64 subshell；<b>subagent 結果改善</b>：達到 <code>maxTurns</code> 上限而停止的 subagent，其輸出現標記為部分完成並提示可透過 <code>SendMessage</code> 接續，而非顯示為已完成；<b>非互動 session 改善</b>：<code>-p</code>、SDK 與雲端 session 現會自動接續因伺服器錯誤、斷線或卡住而中途截斷的回應，不再直接以錯誤結束；<b>用量遙測歸屬改善</b>：workload identity federation session、啟動時執行 <code>apiKeyHelper</code> 期間送出的事件，以及閒置期間登入 token 過期後，皆能更準確歸屬到所屬組織。變更：<code>/code-review</code> 現在 Bedrock、Vertex AI、Foundry、透過 Claude apps gateway，以及遙測或非必要流量關閉時，Claude 亦可自行啟動；<code>/goal</code>：閒置 session 針對每個長時間執行的背景工作最多主動開啟三次確認，下一則訊息會再補三次額度；<code>claude install</code> 與 <code>claude update</code> 現改為將待處理的受管設定同意提示延後到下一次互動 session，不在指令執行中途詢問；來自 claude.ai 同步的 plugin，其 OpenTelemetry plugin 事件的 <code>plugin_id_hash</code> 現反映實際 marketplace，且管理員安裝的 plugin 的 <code>enabled_via</code> 為 <code>admin-install</code>。"},
  ...
  const DATA_CD = [
    {v:"1.37937.3", date:"2026-08-26", cat:"Performance/Bug Fix", sev:"low", title:"維護性更新（無使用者面向變更）", body:"General、Code、Cowork、3P 皆無使用者面向變更，僅內部維護性更新。"},
    {v:"1.37937.2", date:"2026-08-26", cat:"Performance/Bug Fix", sev:"low", title:"維護性更新（無使用者面向變更）", body:"General、Code、Cowork、3P 皆無使用者面向變更，僅內部維護性更新。"},
    {v:"1.37937.1", date:"2026-08-25", cat:"Plugins/MCP", sev:"medium", title:"MCP 遠端連線自動重連修復與 Linear 登入錯誤修正", body:"General：內建 Claude Code CLI 更新至 2.1.246。Code：修復遠端 MCP 伺服器連線中斷後未能恢復，現改為自動重新連線或回報為失敗狀態；修復部分 MCP 伺服器（如 Linear）登入時以「Invalid redirect URI」錯誤失敗。Cowork、3P：無使用者面向變更。"},
    {v:"1.37937.0", date:"2026-08-25", cat:"Plugins/MCP", sev:"high", title:"舊版 Word／Excel 附件支援、Usage 成本估算與多項企業原則新增", body:"General：新增支援舊版 Word .doc 檔案（現可像 .docx 一樣開啟）與 Excel .xlsx／.xls 試算表（現改為以文字附加，先前會被拒絕）；移除組織管理員關閉 Artifacts 時該組織成員分享 chat artifact 的功能（已分享的連結仍可使用）；修復 Cowork 週用量上限用完後 chat 拒絕新訊息；修復排程任務同時設定「每月某天」與「每週某天」（例如「每月 1 號與每週一」）時只在兩者重疊才觸發，現改為任一條件成立即觸發，符合排程說明文字；修復多項 chat 可靠性問題：佇列訊息可能消失或自行送出、服務過載時重試送出會產生重複訊息、回覆尚未送達前重開對話會誤顯示「message wasn't sent」錯誤，以及單次失敗的回應會顯示兩則錯誤訊息；修復 App 在自動更新後偶爾直接將使用者登出。Code：改善 SSH session 可靠性：修復設定的身分金鑰為公鑰時（1Password 設定常見）連線失敗、主機重連期間送出的訊息會在恢復後送達、Claude Code 上傳到主機現可挺過短暫網路中斷、閒置 session 不再誤顯示「Lost connection」卡片、重連時可視需要要求密碼或一次性驗證碼，以及重連不再誤報遺失輸出（除非真的無法復原）；修復任一已儲存 SSH 連線失效會導致所有已儲存 SSH 連線消失；修復 Windows 上背景清理舊 session worktree 時，有時連同 worktree 內 NTFS junction 指向的資料夾內容（如主要 checkout 的 <code>node_modules<\/code>）一併刪除；修復移除某次 claude.ai 匯入執行會重置由該次匯入啟動的 session 中未提交的工作內容，現改為保留該 session 檔案於磁碟；修復組織使用 Claude Code <code>managed-settings.json<\/code> 的 <code>disableSideloadFlags<\/code> 設定時 session 無法啟動，現改為不含桌面版內建 skill 與 plugin 直接啟動；修復開啟或重連含極大 transcript 或大量 subagent 的 session 時記憶體用量過高、視窗空白或無回應。Cowork：新增 Claude in Chrome 側邊面板口述輸入，於擴充功能設定中一次性允許麥克風即可；修復暫存 Google Drive 等雲端同步、尚未下載到 Mac 本機的檔案時出現「upload failed」錯誤，現改為自動下載，並在同步 App 需要啟動時提供更清楚的訊息；修復任務標題列開啟的 Rename、Delete 或 Move 對話框，切換到另一任務並操作該任務時仍保持開啟，現改為切換任務即關閉；修復同時開啟多個都需要確認的檔案時只有最後一個檔案維持附加狀態；修復更新後、session 尚未載入完成前 App 重啟時，已還原的 Cowork 分頁顯示「Try again」錯誤；修復重啟後任務與檔案面板以空白面板保持開啟。3P：新增 <code>mcpToolTimeoutSec<\/code> 設定 MCP 工具呼叫逾時前可執行的秒數，預設 180 秒；新增 <code>organizationInstructions<\/code>：附加到 Chat、Cowork 與 Code session 系統提示的全組織指示（上限 3000 字元），可透過裝置管理、本機設定檔或 bootstrap 回應設定，屬模型會遵循的指引、非強制控管；新增 <code>skipWebFetchPreflight<\/code>，開啟後 Code session 抓取網頁前不再先連線 <code>api.anthropic.com<\/code>，修正該主機被網路封鎖時網頁擷取失敗的問題，預設關閉；新增 <code>userPluginMarketplacesEnabled<\/code> 與 <code>userPluginUploadsEnabled<\/code>，控管成員是否可自行新增 plugin marketplace 或上傳自訂 plugin，關閉時新增選項會隱藏且新增動作會被拒絕，未設定則不改變現況；新增標準 App 已有的 Code tab 功能：含 Show in Files 的 Files 面板、composer 內 emoji 自動完成與行內提示建議、對話中的互動式 MCP app widget，以及讓 Claude 讀取內建終端機面板輸出；新增 Usage 頁面成本估算：<code>inferenceModelPricingEnabled<\/code> 在 token 計數旁顯示以 Anthropic 牌價估算的成本，<code>inferenceModelPricing<\/code> 提供各模型費率、<code>inferenceModelPricingMultiplier<\/code>（0 到 1 之間）可縮放所有估算值，兩個費率鍵將於未來版本生效、之前一律採用牌價估算，預設關閉；新增 chat 內組織自有函式庫的 plugin 與 skill 建議；新增支援回傳使用者名稱或 <code>authtype=Bearer<\/code> 的 plugin marketplace 憑證 helper，讓架在 Bitbucket Data Center 或需 GitLab deploy token 的 marketplace 可完成認證；Claude Code <code>managed-settings.json<\/code> 新增 <code>disableDesktopLocalSessions<\/code> 設定，關閉裝置本機執行的 Code session，Code tab 僅提供如 SSH 的遠端環境，環境選單會將 Local 灰顯並顯示「Disabled by your organization」說明；變更 <code>allowedPluginMarketplaces<\/code>（beta），架在 bootstrap 伺服器自身網域的 <code>url<\/code> marketplace 現可設 <code>credentialKind: \"inferenceCredential\"<\/code>，並沿用 App 既有的 bootstrap 登入方式抓取；變更 <code>builtinToolPolicy<\/code>，除純工具名稱外現亦接受如 <code>Bash(curl *)<\/code> 的參數範圍 Claude Code 權限規則，<code>WebSearch<\/code> 與 <code>WebFetch<\/code> 項目仍須為純工具名稱，非可用規則的項目會以設定錯誤拒絕；變更 gateway device-code 登入，在 gateway 有回傳 email 時顯示該 email 而非電腦登入名稱；變更 Code tab 的檔案面板與 git 面板改為遵循管理員的 <code>allowedWorkspaceFolders<\/code> 設定；修復位於會剝除回應 content type 的 proxy 後方的 Bedrock session，靜默重新送出整個請求且不使用串流，導致每個請求被計費兩次；修復任務表單選取 1M context 列或「Default model」時，Cowork 排程任務卻實際跑在 200K context 模型上，表單現會標示 1M 列；修復內建 Microsoft 365 connector 更新行事曆事件結束時間時送出不合法的 schema；修復同一台電腦有多個 Microsoft 工作帳號時，Windows 上的 Microsoft 365 本機連接器對這些使用者失敗，Reconnect 卡片現會開啟 Windows 帳號選擇器並記住所選帳號，此連接器的使用者需在此次更新後重新連線一次；修復 <code>inferenceModels<\/code> 同時列出含與不含 <code>[1m]<\/code> 後綴的同一模型時，model picker 顯示重複且標示錯誤的 1M context 列。"},
  ...
  const INSP_CC = [
    ...
    {ico:"server", color:"teal", feat:"claude self-hosted-runner — 自架執行環境", ver:"v2.1.224 · 2026-08-07",
     ...
     auto:true},
    {ico:"lock", color:"red", feat:"--restricted 旗標 — CI／無人值守環境的最小權限模式", ver:"v2.1.248 · 2026-08-27",
     desc:"<code>--restricted</code>（或 <code>CLAUDE_CODE_RESTRICTED=1</code>）一鍵移除可執行指令、程式碼與 <code>WebFetch</code> 的內建工具、把檔案工具鎖在工作目錄內、拒絕 <code>bypassPermissions</code>，並忽略使用者／專案／本機設定檔。",
     scen:"<b>場景：</b>銀行內網的排程任務或 CI pipeline 要跑 Claude Code 產文件或做唯讀分析，QA／Backend 團隊改用 <code>--restricted</code> 啟動，即使設定檔或環境變數被竄改也無法逸出唯讀邊界，符合資安稽核對無人值守 agent 的最小權限要求。",
     roles:["backend","qa"],
     d:[
       {h:"為什麼有用", p:"無人值守的排程或 CI session 一旦設定檔被誤改或遭竄改，過去仍可能繼續執行 Bash／WebFetch。<code>--restricted</code> 在啟動當下就砍掉這些高風險工具與 <code>bypassPermissions</code>，並無視任何使用者、專案、本機設定檔，等於在 CLI 層面提供一個不可被組態覆寫的最小權限邊界。"},
       {h:"設定 / 操作", c:"bash", b:"# 排程 / CI 環境改用 restricted 啟動\nCLAUDE_CODE_RESTRICTED=1 claude -p \"整理本週 changelog 摘要\"\n\n# 或明確指定旗標\nclaude --restricted --tools Read,Grep -p \"審閱 diff 並列出風險\""},
       {h:"小技巧", l:["需要的工具（如 <code>WebFetch</code>）可用 <code>--tools</code> 明確列入白名單解除限制", "適合搭配 self-hosted-runner 用在內網無人值守 agent，雙重降低憑證與執行風險"]}
     ],
     auto:true}
  ];
  ...
  const INSP_CI = [
    ...
    {ico:"hook", color:"orange", feat:"Interrupt Hooks", ver:"0.150.0 · 2026-08-26",
     desc:"新增 Interrupt hooks：使用者中斷最上層回合時，可自動執行指令或呼叫 MCP handler。",
     scen:"<b>場景：</b>後端／QA 常在 Codex 執行到一半時中斷（發現方向錯了、要先處理緊急事），過去中斷後留下的暫存檔、鎖定資源或未回滾的 migration 只能手動清；掛上 Interrupt hook 可讓中斷當下自動記錄現場、釋放鎖或回滾未完成的變更。",
     roles:["backend","qa"],
     d:[
       {h:"為什麼有用", p:"以前中斷 turn 只是單純停下來，任何「進行中」的外部狀態（暫存檔、鎖、半套用的 migration）都得靠人工善後。Interrupt hook 讓你在中斷瞬間掛上清理或記錄邏輯，等同幫每次中斷加一道安全網。"},
       {h:"設定 / 操作", c:"toml", b:"# ~/.codex/config.toml\n[hooks.interrupt]\ncommand = \"scripts/cleanup-on-interrupt.sh\"\n# 或改呼叫 MCP handler\n# mcp_handler = \"ops-toolkit.on_interrupt\""},
       {h:"小技巧", l:["回滾未完成的 DB migration 或釋放檔案鎖，避免下一輪 turn 撞到殘留狀態","搭配記錄用途：把中斷當下的 diff／log 存檔，方便回頭比對「中斷前做到哪」"]}
     ], auto: true}
  ];
