...
**🚨 `body` 的寫法直接決定畫面呈現（`index.html` 的 `parseBodyItems` 會照下列分隔符把一段敘述切成條列，摘要列出每項標題、展開後左標題右內容）：**

- **一項一句，項與項之間用「；」**；一個群組結束才用「。」。不可整段連寫成一句。
- **每項寫成「標題：內容」**，標題用 `<b>` 包（`：` 放在 `</b>` 之後，不要包進 `<b>` 裡）。標題寫不出來可以只寫內容，但不要把兩件事塞進同一項。
- **修復／其他／變更這類群組**：群組首項寫「修復：<第一則>」，同群組後續各則直接用「；」接下去（不必每則再寫一次「修復」），群組結束用「。」。
- 指令、旗標、設定鍵用 `<code>` 包；**分隔符不要出現在 `<code>`／`<b>` 內或全形括號內**（切割會略過標籤與括號內部，寫在裡面等於失效）。
